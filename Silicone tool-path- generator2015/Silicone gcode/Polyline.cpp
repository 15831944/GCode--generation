...
void CPolyline::WriteEntities(CStdioFile &myFile, BOOL bSaveMinRectLayout/* =FALSE */)
{
	...
	bool CPolyline::Sparse(double err /* = 0.001 */)
	{
		Point3DAry& pts = m_pts;
		int sz = pts.GetSize();
		if (sz < 5 || err <=0.0)
			return false;     // 如果少于5个点，那么就无需处理

		bool* b = new bool[sz];  // be deleted?
		for (int k=0;k<sz;k++)
		{
			b[k] = false;
		}
		...
		while (e < sz-1)
		{
			...
			while (e < sz-1)
			{
				for (i=s+1;i<e;i++)
				{
					if (GetDist2PtToLine(*pts[i],*pts[s],*pts[e]) > err2)
					{
						break;
					}
				}
