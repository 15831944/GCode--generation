...
public:
	...
	COBBox();
	void Init(double xMin, double xMax, double yMin, double yMax, double zMin=0, double zMax=0);
	CPoint3D GetCenterPt() const;
	bool IsBoxIn(const COBBox& box);
...
double AFX_API_EXPORT GetDistance2(const CPoint3D& pt1,const CPoint3D& pt2);
...
double AFX_API_EXPORT GetDist2PtToLine(const CPoint3D& pt, 
...
*/
enum two_lines_cross_status
{
	TWO_LINES_PARALLEL = 0,
	TWO_LINES_COINCIDE,
	TWO_LINES_CROSS
};
